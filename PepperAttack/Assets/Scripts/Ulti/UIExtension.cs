...
public static class UIExtension
{
    ...
    #region JSON
    public static string toShortJSON(this System.Object thing)
    {
        foreach (var prop in thing.GetType().GetProperties())
        {
            if (prop.GetValue(thing, null) != null)
            {

            }
            // Console.WriteLine("{0}={1}", prop.Name, prop.GetValue(thing, null));
        }
        return "";
    }
    ...
    public static void Log(this System.Object thing)
    {
        Debug.Log("Log " + JsonUtility.ToJson(thing));
    }
    ...
    public static object ConvertToObjectWithoutPropertiesWithNullValues<T>(this T objectToTransform)
    {
        ...
        foreach (var propertyInfo in type.GetFields())
        {
            ...
            //if (value is List<AttackEffect>)
            //{
            //    var _list = (List<AttackEffect>)value;
            //    if (_list.Count <= 0)
            //        continue;
            //}
            //    if(value is in)
            //  Debug.LogError("Convert " + propertyInfo.Name + "_value_" + value);
    ...
    public static string SortWalletAddress(this string address)
    {
        return address.Substring(0, 5) + "....." + address.Substring(address.Length - 5, 5);
        return address;
    }

    public static double TimeServer()
    {
        DateTime _free = DateTime.Now;
        DateTime _utcTime = DateTime.UtcNow;
        DateTime _deadLine = _utcTime.Date.AddDays(1);
        //  return 0;
        // TimeZoneInfo.ConvertTime(DateTime.Now,T);
        //DateTime eastern = TimeZoneInfo.ConvertTimeBySystemTimeZoneId(DateTime.UtcNow, "North Asia Standard Time");
        //DateTime _deadLine = eastern.Date.AddDays(1).AddHours(7);
        double _time = (_deadLine - _utcTime).TotalSeconds;
        return _time;
    }
}
